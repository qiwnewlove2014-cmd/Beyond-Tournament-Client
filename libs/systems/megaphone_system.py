...
class MegaphoneManager:
    ...
    def __init__(self, gameplay):
        ...
        self.eq_slot = self.game.audio_mngr.gen_effect(
            "EQUALIZER",
            ("low_gain", 3.5),          # Warm & punchy bass body (boosted from 2.8)
            ("low_cutoff", 250.0),      # Wider bass band for deeper warmth
    ...
    def refresh_environment_audio(self):
        """MAIN THREAD: repair missing PA sources; keep healthy streams intact.

        True means the sources/effect bindings are ready, not that a broadcast
        is currently audible. Idle speakers must not replay old queued speech.
        """
        if not getattr(self.map, 'megaphone_speakers', None):
            return True
        ...
        def templates_ready():
            data = getattr(self, 'speaker_data', [])
            return (len(data) == len(self.map.megaphone_speakers)
                    and all(valid(item.get('source')) and
                            (item.get('reflection_source') is None or
                             valid(item['reflection_source'])) for item in data))

        def clones_ready(sources):
            if not sources or len(sources) != len(self.speaker_data) * 2:
                return False
            for idx, source in enumerate(sources):
                required = idx % 2 == 0 or self.speaker_data[idx // 2].get('reflection_source') is not None
                if (required or source is not None) and not valid(source):
                    return False
            return True

        try:
            channel = self.voice_channels.get(consts.CHANNEL_MEGAPHONE)
            worker = getattr(channel, 'vc_compression', None)
            if not templates_ready() or worker is None or not worker.is_alive():
                # Do not tear down sources while their old worker still runs.
                if worker is not None and worker.is_alive():
                    worker.close()
                    worker.join(timeout=0.05)
                    if worker.is_alive():
                        return False
                self.setup_megaphone_speakers(force=True)
                channel = self.voice_channels.get(consts.CHANNEL_MEGAPHONE)
                worker = getattr(channel, 'vc_compression', None)
                if not templates_ready() or worker is None or not worker.is_alive():
                    return False
            ...
            healthy = True
            for sender_id, entry in list(getattr(self, 'player_sources', {}).items()):
                try:
                    if not clones_ready(entry.get('sources')):
                        self._remove_megaphone_player(sender_id)
                        sources = self.get_megaphone_player_sources(sender_id)
                        if not clones_ready(sources):
                            healthy = False
                            continue
                        entry = self.player_sources[sender_id]
                    # Restore lost effect sends without changing playback,
                    # gain, speaker position, or the music/instrument locks.
                    for idx, source in enumerate(entry['sources']):
                        if source is None:
                            continue
                        data = self.speaker_data[idx // 2]
                        filters = entry.get('filters', ())
                        filter_obj = filters[idx] if idx < len(filters) else None
                        local_reverb = getattr(self, 'current_player_reverb_slot', None)
                        if local_reverb == 'UNINIT':
                            local_reverb = None
                        for slot, effect in enumerate((self.eq_slot, data.get('reverb_slot'), self.compressor_slot, local_reverb)):
                            self.game.audio_mngr.efx.send(source, slot, effect, filter=filter_obj)
                        if filter_obj is not None:
                            source.direct_filter = filter_obj
                except Exception:
                    healthy = False
            self.request_spatial_refresh()
            return healthy
        except Exception:
            return False
    ...
    def _remove_megaphone_player(self, sender_id, preserve_delay_cache=False):
        ...
        for src in entry['sources']:
            ...
            try:
                ...
                pass
            # Drain only processed buffers; a failed stop must never cause an
            # infinite loop over a still-playing queue. Reuse the shared pool.
            voice_chat._reclaim_source_buffers(src)
            try:
                del src.buffer
            except Exception:
        ...
        # cyal filters have no delete(). Detach FIRST, then return them to the
        # manager pool so a replacement source cannot reuse an attached filter.
        for filter_obj in entry.get('filters', ()):
            if filter_obj is not None:
                self.game.audio_mngr.release_filter(filter_obj)
